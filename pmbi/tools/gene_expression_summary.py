...
def expr_summary_figure(adata, gene, outdir=".", figsize=(6, 8), format="png"):
    ...
    nclust = len(adata.obs["leiden_1.0"].unique())
    ...
    top_1_perc = np.quantile(adata.layers["scale_data"][:, gene_idx], 0.99)
    sc.pl.embedding(
        adata,
        basis="X_umap",
        color=gene,
