...
# %%
def toml_config(path: str) -> Munch:
    """
    Loads and parses a TOML configuration file into a Munch object.

    Args:
        path (str): The file path to the TOML configuration file.

    Returns:
        Munch: A Munch object that contains the configuration data
        from the TOML file, allowing attribute-style access.
    """
...
class PushPullFiles:
    ...
    def __init__(self, src: str, sink: str, input: InputFiles, output: InputFiles):
        if len(input.keys()) == 0 or len(output.keys()) == 0:
            raise NotImplementedError(
                "PushPullFiles does not yet support InputFiles or OutputFiles without names."
            )
    ...
    def _update_paths(self):
        self.input_at_sink = InputFiles(
            fromdict={
                k: os.path.join(self.sink, *path_difference(self.src, v)[1])
                for k, v in self.input_at_source.items()
            }
        )
        self.output_at_sink = OutputFiles(
            fromdict={
                k: os.path.join(self.sink, *path_difference(self.src, v)[1])
                for k, v in self.output_at_source.items()
            }
        )
    ...
    def _mkdir_at_sink(self):
        for dn in [
            os.path.dirname(p)
            for p in itertools.chain(self.input_at_sink, self.output_at_sink)
        ]:
    ...
    def _clean_sink(self):
        ...
        for dn in [
            os.path.join(self.sink, path_difference(self.sink, f)[1][0])
            for f in itertools.chain(self.input_at_sink, self.output_at_sink)
        ]:
            if os.path.isdir(dn):
                shutil.rmtree(dn)
