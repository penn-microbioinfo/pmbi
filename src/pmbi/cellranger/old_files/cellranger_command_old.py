...
CRCONFIG = tomllib.load(
    open(
        os.path.join(os.path.dirname(pmbi.__file__), "cellranger", "config.toml"), "rb"
    )
)
...
def check_multiome_experiments(experiments):
    allmodals = set(
        itertools.chain(*[[xx.type for xx in x.modalities()] for x in experiments])
    )
    ...
    if not all([x == max_modals for x in nmodals]):
        for exp in experiments:
            if len(exp.modalities()) != max_modals:
                maybe_missing_modals = [
                    x
                    for x in experiments
                    if x.identifier == re.sub("[a-z]{1}$", "", exp.identifier)
                ]
                    ...
                    for mm in maybe_missing_modals:
                        for m in mm.modalities():
                            if exp.try_add_modality(m):
                                logging.warning(
                                    f"Added missing modality to experiment {exp.identifier}: ({m.super_sample}, {m.type})"
                                )
                                mm.redundant = True
                    if len(exp.modalities()) != max_modals:
                        logging.warning(
                            f"Unable to find missing modalities for {exp.identifier}. Continuing with only: {[m.type for m in exp.modalities()]}"
                        )
    ...
    for exp in experiments:
        if not exp.redundant:
            ...
            logging.warning(
                f"Skipping redundant experiment for {exp.identifier} with modalities: {[x.type for x in exp.modalities()]}"
            )
...
def parse_keys_multiome(keys):
    ...
    if not all(
        [
            x == [os.path.dirname(y) for y in keys][0]
            for x in [os.path.dirname(x) for x in keys]
        ]
    ):
    ...
    for key in keys:
        ...
        if s is not None:
            ...
            modal = s.group(2)
            print(key, super_sample, sample, modal)
            if not super_sample in super_samples:
                super_samples[super_sample] = [
                    CellrangerModality(super_sample, sample, modal, awsdir)
                ]
            else:
                super_samples[super_sample].append(
                    CellrangerModality(super_sample, sample, modal, awsdir)
                )
        else:
...
class CellrangerCommand(object):
    ...
    def compress_outs(self):
        ...
        if len(self.important_outs) > 0:
            ...
            cmds.append(
                f"tar cvzf {self.archive_name.replace('tar.gz', 'importantOuts.tar.gz')} important_outs/"
            )
        return "\n".join(cmds)
    ...
    def command_line_args_repr(self, exclude: list):
        cli_args = [
            f"--{k.replace('_', '-')} {v}"
            for k, v in self.command_line_args.items()
            if k not in exclude
        ]
...
class CellrangerCountRNA(CellrangerCommand):
    def __init__(self, **kwargs):
        ...
        self.command_line_args["include_introns"] = str(
            self.should_include_introns()
        ).lower()
        self.command_line_args["transcriptome"] = self.get_reference_path()
        self.archive_name = (
            f"{self.command_line_args['sample']}_filtered_feature_bc_matrix.tar"
        )
        self.path_to_outs = (
            f"{self.command_line_args['sample']}/outs/filtered_feature_bc_matrix"
        )
...
class CellrangerCountADT(CellrangerCommand):
    ...
    def __str__(self):
        return "\n".join(
            [
                self.bash_make_libraries_csv(),
                self.command_line_args_repr(exclude=["fastqs", "sample"]),
            ]
        )
...
class CellrangerMulti(object):
    def __init__(self, fastqs, config, experiment, output_key_prefix):
        ...
        self.important_archive_name = (
            f"{self.experiment.identifier}.multi.importantOuts.tar.gz"
        )
    ...
    def print_s3_download_cmds(self):
        ...
        for m in self.experiment.modalities():
            cmds.append(
                f"python /shared-ebs/microbioinfo-aws/scripts/s3-download-multi.py --bucket microbioinfo-storage --prefix {os.path.join(m.awsdir, m.sample)} --chunksize 1.024e8 --pattern '[_]R[0-9]+[_][0-9]+[.]fastq[.]gz$'"
            )
        return "\n".join(cmds)
...
class CellrangerMultiConfigSection(object):
    ...
    def _assess(self):
        if len(self.rows) > 0 and len(self.kvpairs) > 0:
            raise ValueError(
                f"Section {self.name} cannot be composed of rows and key-value pairs."
            )
        else:
            if len(self.rows) == 0 and len(self.kvpairs) == 0:
                raise ValueError(
                    f"Section {self.name} has to have one of: rows or key-value pairs."
                )
            else:
                if len(self.rows) > 0:
                    self.type = "rows"
    ...
    def add_row(self, rowdict: dict):
        if len(self.colnames) == 0:
            ...
            if len(rowdict) != 0:
                raise ValueError(
                    f"Some columns did not appear in colnames for row: {rowdict}"
                )
...
class CellrangerMultiConfig(object):
    ...
    def modality_to_section_header():
        return {"RNA": "gene-expression", "VDJ": "vdj", "ADT": "feature"}

    def modality_to_feature_type():
        return {"RNA": "Gene Expression", "VDJ": "VDJ", "ADT": "Antibody Capture"}

    def add_section(self, name: str, section: CellrangerMultiConfigSection):
        setattr(self, name, section)

    def from_multi_experiment(
        path_to_fastqs, experiment: CellrangerMultiomeExperiment, refs: dict
    ):
...
def cellranger_commands(table, s3_output_prefix="cellranger_matrices"):
    ...
    for row in crtable.itertuples():
        ...
        if row.read_number in ["R1", "read1"]:
            sampleid = re.sub(
                "_S[0-9]+[_]L[0-9]+[_][IR][0-9]+[_][0-9]+[.]fastq[.]gz",
                "",
                row.read_filename,
            )
            ...
            this_sample.append(
                f"python /shared-ebs/pmbi/pmbi/s3/s3-download-multi.py --bucket {bucket} --prefix {read_prefix} --chunksize 1.024e8 --pattern '[_]R[0-9]+[_][0-9]+[.]fastq[.]gz$' && \\"
            )
            ...
            output_uri = (
                f"s3://{bucket}/{s3_output_prefix}/{this_cellranger_cmd.archive_name}"
            )
            this_sample.append(
                f"python /shared-ebs/pmbi/pmbi/s3/s3-multipart-upload.py --uri {output_uri} --partsize 100000000 --nproc 8 --skip_verify_etags --largefile {this_cellranger_cmd.archive_name}"
            )
            this_sample.append(
                f'python /shared-ebs/pmbi/pmbi/s3/s3-multipart-upload.py --uri {output_uri.replace(".tar.gz", ".importantOuts.tar.gz")} --partsize 100000000 --nproc 8 --skip_verify_etags --largefile {this_cellranger_cmd.archive_name.replace('.tar.gz', '.importantOuts.tar.gz')}'
            )
            if sampleid in cmds:
